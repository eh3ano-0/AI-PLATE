...
#با K-means رنگ‌ها رو کم می‌کنه (color quantization) تا مرزها واضح‌تر بشن.
def sharpen_new(img):
    """
...
# یک kernel شارپ‌کننده روی تصویر اعمال می‌کنه (لبه‌ها تیزتر).
...
#روشن‌تر کردن تصویر با PIL (ولی return نداره → با inplace تغییر می‌ده).
...
# زاویه‌ی کجی متن/پلاک رو با HoughLines محاسبه می‌کنه.
...
#تصویر رو با استفاده از زاویه‌ی محاسبه شده صاف می‌کنه.
def deskew(src_img):
    """
...
# خاکستری کردن، باینری کردن، حذف نویز، ضخیم کردن فونت.
def grayscale(image):
    """
...
#حذف نویز با عملگرهای مورفولوژی (dilate → erode → close → median blur).
...
#ساده کردن کانتور با cv2.approxPolyDP.
...
#پیدا کردن کانتور چهارضلعی (مثل کاغذ یا پلاک).
...
#تصحیح پرسپکتیو بر اساس چهار نقطه (برای صاف کردن مستطیل‌ها مثل پلاک).
...
#تبدیل تصویر OpenCV → Qt Pixmap (برای نمایش در PySide6).
...
#تنظیم روشنایی و کنتراست به روش سفارشی.
...
#تغییر سایز دستی (خودش با اندیس‌بندی پیکسل‌ها).
...
#کنار هم گذاشتن چند تصویر (افقی یا عمودی).
